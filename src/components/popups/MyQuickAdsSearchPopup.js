...
import PlatformIcon from '../nonAuth/QuickAds/PlatformIcon';
import moment from 'moment';

const MyQuickAdsSearchPopup = ({sliderValue, setSliderValue, ...props}) => {
  ...
  const filterData = jobsData.filter(item => {
    const title = item.quick_ads_title
      ? item.quick_ads_title.toUpperCase()
      : '';
    return title.includes(search.toUpperCase());
  });
  ...
  const validApplicants =
    filterData?.applicants?.filter(
      applicant => applicant?.status !== 'cancel',
    ) || [];

  const platformData = filterData?.platform;

  const remainingSlots = filterData?.apply_limit - validApplicants.length;

  const todayDate = moment().format('DD MMMM YYYY');
  const endDate = moment(filterData?.task_start_date).format('DD MMMM YYYY');
...
export default MyQuickAdsSearchPopup;
