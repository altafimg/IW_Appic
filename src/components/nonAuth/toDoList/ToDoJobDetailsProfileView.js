...
const ToDoJobDetailsProfileView = ({data, loading}) => {
  const navigation = useNavigation();

  const jobData = {
    _id: data?.user_id?._id,
    profile_name: data?.user_id?.profile_name,
    profile_picture: data?.user_id?.profile_picture,
  };
        ...
        <View style={styles.profileContainer}>
            ...
            <View style={styles.profilePickView}>
              <TouchableOpacity
                ...
                <View style={styles.profilePickView}>
                  ...
                  data?.user_id?.profile_picture == undefined ? (
                    <Image source={userImage} style={styles.profileImage} />
                  ) : (
                    <Image
                      source={{uri: data?.user_id?.profile_picture}}
          ...
          <View style={styles.profileRightContainer}>
            <View style={styles.userNameView}>
              ...
              {data?.user_id?.user_verify_status === 'verified' ? (
                <SVG.BlueTick />
              ) : (
                ''
              )}
            ...
            <Text style={styles.baseTitle}>
              {data?.user_id?.user_role?.charAt(0).toUpperCase() +
                data?.user_id?.user_role?.slice(1)}
            </Text>

            {data?.user_id?.language?.map((item, index) => {
              return (
...
export default ToDoJobDetailsProfileView;
